...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("hotshotcoin"))
        return false;
...
{
    // Convert hotshotcoin:// to hotshotcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hotshotcoin://"))
    {
        uri.replace(0, 12, "hotshotcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HotShotCoin.lnk";
}
...
{
    return GetAutostartDir() / "HotShotCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HotShotCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  HotShotCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("HotShotCoin-Qt"));
