...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	if (IsProofOfWork())
	{
		int64_t nReward = GetProofOfWorkReward(pindex->pprev->nHeight, nFees);
	...
	if (IsProofOfStake())
	{
        ...
        uint64_t nCalculatedStakeReward = GetProofOfStakeReward(pindex->pprev->nHeight, nCoinAge, nFees);
