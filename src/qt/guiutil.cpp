...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("glovecoin"))
        return false;
...
{
    // Convert glovecoin:// to glovecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("glovecoin://"))
    {
        uri.replace(0, 12, "glovecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GloveCoin.lnk";
}
...
{
    return GetAutostartDir() / "GloveCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GloveCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  GloveCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GloveCoin-Qt"));
