...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(nHeight, 0);
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "HotshotCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "HotshotCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "HotshotCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HotshotCoin is downloading blocks...");
